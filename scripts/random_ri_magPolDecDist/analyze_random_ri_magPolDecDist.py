...
def mswResonanceVariablesCreate(hierarchy, E_MeV, distAll_km, deltaMsqr_MeV2, theta, Vcc_km_1, Vnc_km_1, hamTot_km_1):
    # ============================
    # Shortcuts
    ...
    M_MeV    = np.sqrt( ((Delta_MeV* np.sin(2* theta))**2)+ ((Delta_MeV* np.cos(2* theta) - Vcc__MeV/2)**2) )
    Mbar_MeV = np.sqrt( ((Delta_MeV* np.sin(2* theta))**2)+ ((Delta_MeV* np.cos(2* theta) + Vcc__MeV/2)**2) )
    ...
    theta_M   = np.pi/2 + np.arctan2(Delta_MeV* np.sin(2* theta), (Delta_MeV* np.cos(2* theta)- Vcc__MeV/2))/2
    thetabar_M=           np.arctan2(Delta_MeV* np.sin(2* theta), (Delta_MeV* np.cos(2* theta)+ Vcc__MeV/2))/2
    ...
    if hierarchy == 1:
        ...
        tmp= np.array(np.where((((np.roll(np.sign(hamTot_km_1[:,0,0,0]- hamTot_km_1[:,0,1,1]), 1) - np.sign(hamTot_km_1[:,0,0,0]- hamTot_km_1[:,0,1,1])) != 0)) == 1)).flatten()
    else:
        # For Inverted Hierarchy
        tmp= np.array(np.where((((np.roll(np.sign(hamTot_km_1[:,0,2,2]- hamTot_km_1[:,0,3,3]), 1) - np.sign(hamTot_km_1[:,0,2,2]- hamTot_km_1[:,0,3,3])) != 0)) == 1)).flatten()
    ...
    if tmp.size == 0:
        ...
        if tmp[0] == 0: 
            ...
            exit(tLog.EXIT + "The first element of tmp is not zero. It is " + str(tmp[0]))
        # Create Zero Arrays
        r_resMSWs_km_idx= np.zeros(len(tmp), dtype=int); r_resMSWs_km=np.zeros(len(tmp)); 
        adiabaticity_MSW= np.zeros(len(tmp)); P_LZ_MSW= np.zeros(len(tmp)); itTmp=0
        ...
        for it1 in range(len(adiabaticity_MSW)):
            adiabaticity_MSW[it1]= (((deltaMsqr_MeV2/ (2* E_MeV))* np.sin(2* theta))**2)/ Vcc_gradient_MeV2[r_resMSWs_km_idx[it1]]
        # ============================
        ...
        print(tLog.INFO + "Landau Zener Transition Probability is(are) " + str(P_LZ_MSW) + ".")
        # ============================
...
def sfpResonanceVariables(hierarchy, E_MeV, distAll_km, deltaMsqr_MeV2, Vcc_km_1, Vnc_km_1, muB_km_1, hamTot_km_1):
    # ============================
    # Shortcuts
    Delta_MeV= deltaMsqr_MeV2/ (4* E_MeV)
    Vcc__MeV= Vcc_km_1* km_1__MeV
    ...
    if hierarchy == 1:
        # For Normal Hierarchy
        tmp= np.array(np.where((((np.roll(np.sign(hamTot_km_1[:,0,1,1]- hamTot_km_1[:,0,2,2]), 1) - np.sign(hamTot_km_1[:,0,1,1]- hamTot_km_1[:,0,2,2])) != 0)) == 1)).flatten()
    else:
        # For Inverted Hierarchy
        tmp= np.array(np.where((((np.roll(np.sign(hamTot_km_1[:,0,0,0]- hamTot_km_1[:,0,3,3]), 1) - np.sign(hamTot_km_1[:,0,0,0]- hamTot_km_1[:,0,3,3])) != 0)) == 1)).flatten()
    ...
    if tmp.size == 0:
        ...
        denominator_km_2= np.real(np.abs(np.gradient(((-2* Vnc_km_1)- Vcc_km_1), distAll_km[1]-distAll_km[0])))
        adiabaticity_SFP = (np.real(muB_km_1[r_resSFP_km_idx])**2)/denominator_km_2[r_resSFP_km_idx]
        # ============================
...
def analyzeAllResults():
    ...
    noOfSim=len(next(osCommand.walk(RESULTS_DIR))[1]) # https://stackoverflow.com/questions/29769181/count-the-number-of-folders-in-a-directory-and-subdirectories
    print(tLog.INFO+'Number of simulations is '+str(noOfSim)+'.')
    # ============================

    # ============================
    E_MeV= np.zeros(noOfSim); TraceTerm= np.zeros(noOfSim); distINIT_km= np.zeros(noOfSim)
    # ============================
    ...
    for it_sim in range(1, noOfSim+1):
        ...
        r_resMSWs_km_idx, r_resMSWs_km, adiabaticity_MSW, P_LZ_MSW_tmpArray=\
            mswResonanceVariablesCreate(hierarchy, E_MeV[it_sim-1], distAll_km, deltaMsqr_MeV2, theta, Vcc_km_1, Vnc_km_1, hamTot_km_1)
        print(tLog.INFO+ '============================')
        r_resSFP_km_idx, r_resSFP_km, adiabaticity_SFP, P_LZ_SFP=\
            sfpResonanceVariables(hierarchy, E_MeV[it_sim-1], distAll_km, deltaMsqr_MeV2, Vcc_km_1, Vnc_km_1, muB_km_1, hamTot_km_1)
        print(tLog.INFO+ '============================\n')
        '''
        # ============================
        ...
        rhoFlavAll_finalAveragedDiag[it_sim-1,0]= np.real(rhoFlavAll[-avIndex:,0,0,0].mean())
        rhoFlavAll_finalAveragedDiag[it_sim-1,1]= np.real(rhoFlavAll[-avIndex:,0,1,1].mean())
        rhoFlavAll_finalAveragedDiag[it_sim-1,2]= np.real(rhoFlavAll[-avIndex:,0,2,2].mean())
        rhoFlavAll_finalAveragedDiag[it_sim-1,3]= np.real(rhoFlavAll[-avIndex:,0,3,3].mean())
        ...
        #plt.plot(distAll_km, rhoFlavAll_finalAveragedDiag[it_sim-1,flavType]* np.ones(len(distAll_km)), label='rho11_Final_Averaged')
        #plt.legend()
        #plt.show()
        #plt.close('all')
        # ============================
        ...
        if totFlav == 2:
            if dim_rho_2totFlav_Bool: # TraceTerm = TraceTermb
                if DATA_parametersDic['neutrino_distributionParameter'] == 1:
                    ...
                    luminosityArray_MeV_km[0]= DATA_parametersDic['luminosity_e']* erg_s_TO_MeV_km
                    luminosityArray_MeV_km[1]= DATA_parametersDic['luminosity_mu']* erg_s_TO_MeV_km
                    luminosityArray_MeV_km[2]= DATA_parametersDic['luminosity_eb']* erg_s_TO_MeV_km
                    luminosityArray_MeV_km[3]= DATA_parametersDic['luminosity_mub']* erg_s_TO_MeV_km
                    ...
                    for i3 in range(totFlav*2):
                        constant= (km_1__MeV**2)*(DATA_parametersDic['TraceTerm'][0] * (1 / (4 * (distAll_km[0]**2) * np.pi**2))* luminosityArray_MeV_km[i3] / (3.1514 * tempArray_MeV[i3]))
                        rhoFlavAll_initialDiag[it_sim-1,i3] = rhoFlavAll_initialDiag[it_sim-1,i3]/ constant
                        rhoFlavAll_finalAveragedDiag[it_sim-1,i3]= rhoFlavAll_finalAveragedDiag[it_sim-1,i3]/ constant
                        print(constant)
                        input('')
        '''
            ...
            if DATA_parametersDic['neutrino_distributionParameter'] == 1:
                ...
                for i2 in range(len(E_MeV)):
                    for i3 in range(totFlav*2):
                        ...
                        rho[i2, i3, i3]  = (1/ (4 * (distINIT_km[i2]**2) * np.pi**2))* distributionInit_nu_flav[i2,i3, i3]* luminosityArray_MeV_km[i3] / (3.1514 * tempArray_MeV[i3])
                ...
                hold_E= np.zeros(int(noOfSim/3)); hold_energySpec= np.zeros((int(noOfSim/3), 4, 2))
                for it in range(int(noOfSim/3)):
                    hold_E[it]= E_MeV[it*3]
                    for it2 in range(4):
                        hold_energySpec[it,it2]= ([np.min(energySpec[1,it*3:(it+1)*3,it2]), np.max(energySpec[1,it*3:(it+1)*3,it2])])
                   
                plt.plot(E_MeV, energySpec[0,:,0], color='r', linestyle='dashed', label=r'$\rho_{i,ee}$')
                plt.plot(E_MeV, energySpec[0,:,1], color='k', linestyle='dashed', label=r'$\rho_{i,xx}$')
                plt.plot(E_MeV, energySpec[0,:,2], color='b', linestyle='dashed', label=r'$\rho_{i,ebeb}$')
                plt.plot(E_MeV, energySpec[0,:,3], color='g', linestyle='dashed', label=r'$\rho_{i,xbxb}$')

                plt.scatter(E_MeV, energySpec[1,:,0], color='r', s=1, label=r'$\rho_{ee}$')
                plt.scatter(E_MeV, energySpec[1,:,1], color='k', s=1, label=r'$\rho_{xx}$')
                plt.scatter(E_MeV, energySpec[1,:,2], color='b', s=1, label=r'$\rho_{ebeb}$')
                plt.scatter(E_MeV, energySpec[1,:,3], color='g', s=1, label=r'$\rho_{xbxb}$')

                #plt.fill_between(hold_E, hold_energySpec[:,0,0], hold_energySpec[:,0,1], color='r', alpha=0.6)
                #plt.fill_between(hold_E, hold_energySpec[:,1,0], hold_energySpec[:,1,1], color='k', alpha=0.6)
                #plt.fill_between(hold_E, hold_energySpec[:,2,0], hold_energySpec[:,2,1], color='b', alpha=0.6)
                #plt.fill_between(hold_E, hold_energySpec[:,3,0], hold_energySpec[:,3,1], color='g', alpha=0.6)
                plt.legend()        
    ...
    # If Fermi Dirac Distribution is used
    if DATA_parametersDic['neutrino_distributionParameter'] == 1:
        plt.plot(E_MeV, rhoFlavAll_initialDiag[:,0]* TraceTerm, color='r', linestyle='dashed', label=r'$\rho_{i,ee}$')
        plt.plot(E_MeV, rhoFlavAll_initialDiag[:,1]* TraceTerm, color='k', linestyle='dashed', label=r'$\rho_{i,xx}$')
        plt.plot(E_MeV, rhoFlavAll_initialDiag[:,2]* TraceTerm, color='b', linestyle='dashed', label=r'$\rho_{i,ebeb}$')
        plt.plot(E_MeV, rhoFlavAll_initialDiag[:,3]* TraceTerm, color='g', linestyle='dashed', label=r'$\rho_{i,xbxb}$')

        plt.scatter(E_MeV, rhoFlavAll_finalAveragedDiag[:,0]* TraceTerm, color='r', s=0.3, label=r'$\rho_{ee}$')
        plt.scatter(E_MeV, rhoFlavAll_finalAveragedDiag[:,1]* TraceTerm, color='k', s=0.3, label=r'$\rho_{xx}$')
        plt.scatter(E_MeV, rhoFlavAll_finalAveragedDiag[:,2]* TraceTerm, color='b', s=0.3, label=r'$\rho_{ebeb}$')
        plt.scatter(E_MeV, rhoFlavAll_finalAveragedDiag[:,3]* TraceTerm, color='g', s=0.3, label=r'$\rho_{xbxb}$')
        plt.legend()
        plt.show()
        plt.close()
        exit()
    ...
    # Scattering Graph
    plt.scatter(E_MeV, rhoFlavAll_finalAveragedDiag[:,0], color='k', s=0.3, label=r'$\rho_{ee}$')
    plt.legend()
    plt.show()
    plt.close()

    plt.scatter(E_MeV, rhoFlavAll_finalAveragedDiag[:,1], color='k', s=0.3, label=r'$\rho_{xx}$')
    plt.legend()
    plt.show()
    plt.close()

    plt.scatter(E_MeV, rhoFlavAll_finalAveragedDiag[:,2], color='k', s=0.3, label=r'$\rho_{ebeb}$')
    plt.legend()
    plt.show()
    plt.close()

    plt.scatter(E_MeV, rhoFlavAll_finalAveragedDiag[:,3], color='k', s=0.3, label=r'$\rho_{xbxb}$')
    plt.legend()
    plt.show()
    plt.close()
